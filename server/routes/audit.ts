...
router.post("/audits/:auditId/generate-executive-summary", requireCompanyAuth, requireRole(["CompanyAdmin", "Auditor"]), async (req: AuthenticatedCompanyRequest, res) => {
  try {
    ...
    const isExternal = audit.auditType === "EXTERNAL";
    const auditedEntityName = audit.entityName || (isExternal ? 'the audited entity' : company?.legalName) || 'Unknown Provider';
    const auditedEntityAbn = audit.entityAbn || (!isExternal ? company?.abn : '') || '';
    const auditedEntityAddress = audit.entityAddress || '';
    const auditingOrgName = isExternal ? (audit.externalAuditorOrg || company?.legalName || '') : (company?.legalName || '');
    const auditingOrgAuditor = isExternal ? audit.externalAuditorName : '';
...
AUDIT DETAILS:
- Entity Being Audited: ${auditedEntityName}${auditedEntityAbn ? ` (ABN: ${auditedEntityAbn})` : ''}${auditedEntityAddress ? `\n- Entity Address: ${auditedEntityAddress}` : ''}
- Audit Type: ${audit.auditType}${isExternal ? `\n- Auditing Organisation: ${auditingOrgName}${auditingOrgAuditor ? `\n- Lead Auditor: ${auditingOrgAuditor}` : ''}` : ''}
